...
subtitle <- paste0(
  ...
  team_color_1,
  "'><b>Dark blue</b> bar shows % of team dropbacks where player ran a route<br>",
  "<span style = 'font-family:Chivo; color:",
  team_color_2,
  "'><b>Green bar</b> shows the % of team dropbacks where the player was targeted"
)
...
sea_data <- sea_data %>% 
  ...
  mutate(label_position = case_when(
    measure == "route_percent" ~ week_number - 0.135,
...
test_plot <- ggplot() +
  ...
  scale_fill_manual(values = c("#0c2340",
                               "#00843d")) +
  scale_color_manual(values = c("#0c2340",
                               "#00843d")) +
...
ggsave(test_plot,
       filename = "Plots without Logos/sea utilisation facet.png",
       units = "in",
       height = 9,
       width = 9)
...
seattle_pass_game_facet_with_logo <- add_logo(
  plot_path = "Plots without Logos/sea utilisation facet.png", # url or local file for the plot
...
magick::image_write(seattle_pass_game_facet_with_logo,
                    "Plots with Logos/seattle_pass_game_facet_with_logo.png")
