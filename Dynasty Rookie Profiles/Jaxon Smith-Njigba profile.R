...
player_full_name <- "Jaxon Smith-Njigba"
player_first_name <- "Jaxon"
player_surname <- "Smith-Njigba"
player_position <- "WR"
player_age <- "20.9 YEARS"
...
player_armlength <- ""
player_wingspan <- 
headshot_url <- "https://a.espncdn.com/combiner/i?img=/i/headshots/college-football/players/full/4430878.png&w=350&h=254"

# College History
years <- c(2020,2021,2022)
college <- c("Ohio State",
             "Ohio State",
             "Ohio State")
...
data <- read_csv(paste0(
  ...
  mutate(value = case_when(
    measure == "ms_rush_yd" ~ round(value, 2),
    measure == "ms_touches" ~ round(value, 2),
    measure == "dom" ~ round(value, 2),
    TRUE ~ round(value, 1)))  %>% 
  select(-x5,-x6,-x7)
...
data <- data %>% 
  mutate(x_position = case_when(
    measure == "games" ~ 2.25,
    measure == "ms_rush_yd" ~ 3,
    measure ==  "ms_touches" ~ 3.75,
    measure == "dom" ~ 4.5,
    measure == "rec_yd_pta" ~ 5.25,
    measure == "adj_ypp" ~ 6
  )) %>% 
  ...
  mutate(column_headers = case_when(
    measure == "games" ~ " <br> <br> GAMES",
    measure == "ms_rush_yd" ~ " <br>SHARE OF<br>RUSH YARDS",
    measure ==  "ms_touches" ~ " <br>SHARE OF<br>TOUCHES",
    measure == "dom" ~ " <br>DOMINATOR<br>RATING",
    measure == "rec_yd_pta" ~ "RECEIVING YARDS<br>PER TEAM<br>ATTEMPT",
    measure == "adj_ypp" ~ " <br>ADJUSTED YARDS<br>PER PLAY"
  )) 
...
player_annotation <- paste0(
  ...
  "<br>",
  "<span style = 'font-family:Chivo;font-size:8pt;color: #999999'>",
  "POSITION: ",
...
# Adding percents for relevant columns ====

data_percent <- data %>%
  filter(measure == "ms_touches" | measure == "ms_rush_yd" | measure == "dom") %>% 
  mutate(value_percent = percent(value))
...
profile_summary_plot <- ggplot() +
  scale_x_continuous(limits = c(0.25, 6.35),
                     breaks = NULL,
                     expand = c(0,0)) +
  ...
  geom_shadowtext(data = data %>% 
                    filter(measure != "ms_rush_yd") %>% 
                    filter(measure != "ms_touches") %>% 
                    filter(measure != "dom"),
                  aes(x = x_position,
                      y = y_position,
                      label = value),
                  ...
                  show.legend = FALSE,
                  size = 2.75) + 
  geom_shadowtext(data = data_percent,
                  aes(x = x_position,
                      y = y_position,
                      label = value_percent),
                  color = "#333333",
...
ggsave(profile_summary_plot,
       filename = paste0("plots with no logos/",
                         player_first_name,
                         "_",
                         player_surname,
...
profile_summary_plot_with_logo <- add_logo(
  plot_path = paste0("plots with no logos/",
                     player_first_name,
                     "_",
                     player_surname,
...
comparatives_plot <- ggplot() +
  ...
  geom_vline(xintercept = 0,
             color = "#333333") +
  labs(title = paste0(player_full_name),
       subtitle = paste0(
         "<span style = 'font-size:16pt'><b>Comparable WR rookie season performance</b></span><br>",
         "Plot shows Points Per Game performance during rookie season for closest<br>comparable players in previous drafts"),
       caption = caption_2,
       y = NULL,
...
comparatives_plot_with_logo <- add_logo_2(
  plot_path = paste0("plots with no logos/",
                     ...
                     "_",
                     player_surname,
                     "comparatives_plot.png"), # url or local file for the plot
...
magick::image_write(comparatives_plot_with_logo,
                    paste0("plots with logos/",
                           ...
                           "_",
                           player_surname,
                           "_comparatives_plot_with_logo.png"))
