...
data <- data %>% 
  mutate(x_position = case_when(
    measure == "games" ~ 2.6,
    measure == "rush_avg" ~ 3.45,
    measure ==  "rec_avg" ~ 4.3,
    measure == "rec_yd_pta" ~ 5.15,
    measure == "adj_ypp" ~ 6,
    measure == "games" ~ 0
  )) %>% 
  ...
  mutate(column_headers = case_when(
    ...
    measure == "rush_avg" ~ "AVERAGE YARDS<br>PER RUSH<br>ATTEMPT",
    measure ==  "rec_avg" ~ " <br>AVERAGE YARDS<br>PER RECEPTION",
    measure == "rec_yd_pta" ~ "REC. YARDS<br>PER TEAM<br>ATTEMPT",
...
profile_summary_plot <- ggplot() +
  scale_x_continuous(limits = c(0.2, 6.4),
                     breaks = NULL,
                     expand = c(0,0)) +
...
comparatives_plot <- ggplot() +
  ...
  labs(title = paste0(player_full_name),
       subtitle = paste0(
         "<span style = 'font-size:16pt'><b>Comparable RB rookie season performance</b></span><br>",
...
comparatives_plot_with_logo <- add_logo_2(
  plot_path = paste0("plots with no logos/",comparatives_plot_name,".png"), # url or local file for the plot
